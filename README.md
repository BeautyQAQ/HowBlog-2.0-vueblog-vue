...
当前已对齐后端契约 **5.0.0 / revision 7**，后端提交 `f983d22`，同步状态见 [backend-api-sync.json](backend-api-sync.json)。后续先读取同级后端工程 `docs/frontend-api-status.json`，再核对 changelog 和当前契约。该版本以当前契约为准，旧版项目指南中的 `X-User-Id` 和 `/im?user=` 已失效。
...
访问令牌即将到期时提前续期；同页面单飞，跨标签页通过 Web Locks 串行化，并同步 LocalStorage。刷新开始前持久化处理中标记，崩溃、超时或结果不确定时清理本地凭据并要求重新登录，不重试旧刷新凭据。生产需 HTTPS/WSS 和支持 Web Locks 的浏览器；不支持 Web Locks 时仍可登录，但到续期时要求重新登录，不进行不安全的并发刷新。LocalStorage 属于敏感存储，部署需防范 XSS，不记录或输出令牌。

HTTP 401 在同一会话内最多续期并重放一次，切换账号后不重放旧请求。HTTP 200 的业务码 `20003` 和 HTTP 403 仅提示权限不足。退出只有服务端确认成功后才清空本地凭据并关闭连接；退出失败（包括 500）保留会话并明确提示未完成，允许重试退出。
...
检查原子凭据保存、单飞与跨标签页刷新、刷新超时和崩溃标记、绝对到期、退出失败保留状态、401 重放与账号切换保护、错误响应（含 403 保留登录态）、分页边界、表单空白校验、评论权限及 WebSocket URL/身份/重连。该检查使用隔离依赖，不替代真实后端 MySQL/Redis 联调。

2026-09-22 revision 7 验收：后端快进更新至 `f983d22`；JDK 21 下 Maven 全模块 `install` 成功，118 项测试中 116 项通过、2 项外部数据库测试跳过。前端契约测试和生产构建通过，构建保留依赖包体积警告。三个后端服务已统一重启，前端继续运行于 `http://localhost:8080/`。

- 经前端代理验证文章、标签和评论公开查询，未知刷新/退出凭据返回 401/20003。
- 使用页面现有测试账号真实登录；两个标签页共 16 个并发续期调用仅产生 1 次刷新请求，两个页面均持有有效新凭据。
- 聊天身份验证通过，令牌轮换后旧连接关闭、新连接建立。
- 浏览器退出成功后本地凭据清空；同一旧令牌在三个服务均返回 401，刷新被拒绝，重复退出返回 200。
- 本轮未执行迁移、角色授权或业务数据增删。测试会话已撤销，认证会话及已消费刷新凭据记录按后端既有逻辑保留。文章/评论 CRUD、Redis 点赞和管理员写入未在 revision 7 重跑；以下为 revision 6 历史验收。
